...
describe('RaceEntrySale', function () {
    ...
    context('purchase quantity is 1', function () {
        it('should purchase successfully', async function () {
            ...
            const receipt = await this.contract.purchaseFor(purchaser, sku, quantity, paymentToken, [gameSessionId], {
                from: operator,
                value: price.eth,
            });

            expectEvent.inTransaction(receipt.tx, this.contract, 'Purchased', {
                purchaser: purchaser,
                operator: operator,
                sku: web3.utils.padRight(sku, 64),
                paymentToken: paymentToken,
                quantity: quantity,
                extData: ['0x' + price.eth.toString(16, 64), web3.utils.padRight(gameSessionId, 64)],
            });
        });
    });
    ...
    context('purchase quantity is not 1', function () {
        it('should revert', async function () {
            await expectRevert(
                this.contract.purchaseFor(purchaser, sku, Two, EthAddress, [gameSessionId], {
                    from: operator,
                    value: price.eth,
                }),
